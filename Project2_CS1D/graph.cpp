...
void Graph::BFS(Vertex *vertex)
{
    ...
    while (!queue.empty())
    {
        ...
        queue.pop();
        sort(adjacencyList[vertex].begin(), adjacencyList[vertex].end());
        qDebug() << QString::fromStdString(vertex->getName()) << endl;
...
void Graph::Dijkstra(Vertex *startingPoint)
{
    double  distance;        // Distances from vertex to starting point
    ...
    while(!unvisted.empty())
    {
        ...
        qDebug() << QString::fromStdString(unvisted.top().getSource()->getName()) << endl;
        qDebug() << unvisted.top().getWeight() << endl;
    }
